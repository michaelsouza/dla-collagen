...
void run_dla(int tmax, int num_bind, char mode, unsigned int seed) {
  ...
  // init data file
  FILE *fid;
  char line[100];  // Assumindo um comprimento máximo de linha de 100 caracteres
  
  // Verificar se o arquivo existe
  if ((fid = fopen(arquivo, "r")) == NULL) {
      printf("O arquivo não existe. Criando o arquivo...\n");
      
      // Criar o arquivo
      fid = fopen(arquivo, "w");
      
      // Verificar se a criação do arquivo falhou
      if (fid == NULL) {
          printf("Falha ao criar o arquivo.\n");
          return;  // Sair do programa com um status de erro
      }
      
      fid = fopen(arquivo, "w");

      // Executar o bloco de código para arquivo vazio aqui
      ...
      // O arquivo já existe, então pode prosseguir
      
      // Fechar o arquivo, pois ele foi aberto apenas para verificação
      fclose(fid);
      ...
      fseek(fid, 0, SEEK_END);  // Mover o ponteiro do arquivo para o fim
      if (ftell(fid) == 0) {  // Verificar se o tamanho do arquivo é 0
          ...
          printf("lendo arquivo incompleto\n");
          fid = fopen(arquivo, "r");
          ...
          // Ler cada linha, exceto a última, salvando os valores
          while (fgets(line, sizeof(line), fid) != NULL) {
            // printf("entrou\n");
            if (!feof(fid)) {  // Verificar se não é a última linha
              // Processar a linha aqui (por exemplo, imprimir)
              printf("%s", line);

              //read line and save data
              sscanf(line, "uid: %d %d %d %d", &uid, &x, &y, &z);
              printf("%d\n", uid);
              fibers.push_back(fiber_t(uid, height, x, y, z));
              kdt.add(uid, fibers);
            }
          ...
          // Abrir o arquivo no modo leitura e escrita
          FILE* file;
          char linha[100];
          long int posicao_ultima_linha = -1;

          file = fopen(arquivo, "a+");
          // Percorrer o arquivo linha por linha
          while (fgets(linha, sizeof(linha), file) != NULL) {
              posicao_ultima_linha = ftell(file);
          }
          // Voltar para a posição da última linha
          fseek(file, posicao_ultima_linha, SEEK_SET);
          // Sobrescrever a última linha com uma string vazia
          fputs("", file);
          ...
          //printf("%d\n", num_bind);
          //printf("uid %d\n", uid);
          printf("Última linha excluída com sucesso.\n"); 
