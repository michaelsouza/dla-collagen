...
class Logger:
    ...
    def save(self, fn, k):
        ...
        with open(fn_, 'a') as fid:
            ...
            if k == 0:
                fid.write('f,num_active_particles,num_deleted_particles,num_deleted_rods\n')
                
            for i in range(len(self.F)):
                a = self.num_active_particles[0]- self.num_active_particles[i]
                fid.write(f'{self.F[i]},{self.num_active_particles[i]},{str(a)},{self.num_deleted_rods[i]}\n')
...
def stress_strain(ssd: StressStrainData, verbose: bool = False) -> Logger:
    ...
    while True:
        ...
        if len(active_rids) == 0:    
            logger.log(F, 0, len(all_deleted_rids))
            break
...
def main():
    ...
    parser.add_argument('-n', type=int, help='number os repetitions for statistic', default=1000)
    ...
    fn_log = fn.replace('.dat', f'_m_{m}.txt')
