\documentclass[11pt,a4paper]{article} % Classe 'book' para documentos grandes, com opção de tamanho de fonte e papel
\usepackage[utf8]{inputenc} % Codificação de caracteres
\usepackage[T1]{fontenc} % Codificação de fonte
\usepackage{lmodern} % Fonte moderna que suporta bem o zoom
\usepackage[portuguese]{babel} % Suporte para idiomas
...
\usepackage{tabularx, booktabs} % Pacotes para tabelas avançadas
\newcolumntype{Y}{>{\centering\arraybackslash}X} % Nova coluna centrada para tabularx
\usepackage{amsmath, amsfonts, amssymb} % Para fórmulas matemáticas
\usepackage{hyperref} % Para hiperlinks no documento
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    filecolor=magenta,      
...
\usepackage{float} % Melhor controle de elementos flutuantes
\usepackage{pdfpages} % Para incluir páginas de outro PDF, útil para apêndices
\usepackage{geometry} % Para configuração personalizada das margens
\usepackage{longtable} % Para tabelas que ocupam mais de uma página
\usepackage{pdflscape} % Para páginas em modo paisagem
\usepackage[section]{placeins} % Garante que as figuras fiquem na seção onde foram definidas
\usepackage{microtype} % Melhora a justificação do texto e o preenchimento de linha
...
pelo agregado ou atingirem uma distância \(2R\) em relação ao centro, caso em que a simulação é reiniciada. Esse processo está representado na Figura \ref{M1}.  
    ...
    \begin{figure}[H] 
        ...
        agregado ou atingirem uma distância \(2R\) da \textit{seed}, fora da zona de interesse, o que leva à reinicialização  
        da simulação.}
        \label{M1} 
    ...
    segundos vizinhos no plano X-Z, enquanto no eixo Y, o movimento é restrito para frente e para trás. Na realidade, as moléculas de  
    colágeno agregam-se lateralmente e em posições escalonadas, com um comprimento característico \(D = 67 \, \text{nm}\). Assim, uma  
    molécula é capturada pelo agregado somente em posições específicas, que são múltiplos de \(D = 4\), em relação a uma molécula já pertencente ao  
...
\subsection{Propriedades Mecânicas}  
    ...
    \section{Resultados}
        ...
        \begin{table}[H] % Adiciona [H] para forçar a tabela a aparecer na posição onde foi inserida no texto
            \caption{Insira o título da tabela aqui} % Adiciona um título à tabela
            \label{tab:unique_label} % Adiciona um identificador único para referências cruzadas
        ...
        \begin{table}[H]
            ...
            \centering
            \label{tab3} % O label deve vir depois do caption para referenciar corretamente
            \begin{tabular}{ll}
                \hline
