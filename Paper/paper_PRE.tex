...
For a given force level $F$, we evaluate the removal probability $P_{R}$ (Eq.~\ref{eq4}) for every molecule currently in the backbone and compare it with an independent random variable $u\in[0,1]$. If $u < P_{R}$, the molecule is removed. When molecules detach, the local cross-sectional stresses $\sigma(i)$ and coordination numbers $K$ are immediately updated across the remaining backbone, which in turn alters $P_R$ for neighboring molecules. Consequently, sweeps at the same constant force level $F$ are iterated repeatedly to allow stress redistribution and cascading failures to proceed. This iterative process continues until a sweep completes with zero additional molecular removals, indicating that the load-bearing backbone has reached a mechanically stable equilibrium state for force $F$. At the end of this relaxation process, the set of all molecules removed at force $F$ is partitioned into spatially contiguous clusters of nearest neighbors; each such cluster constitutes an individual avalanche of size $s$. Multiple independent avalanches may therefore be recorded at a single force level. Only after full relaxation is established at force $F$ is the external force incremented by $\Delta F = 0.5$, and the iterative procedure is repeated. Rupture is defined as the appearance of a completely void cross-section, signaling complete failure of the continuous load path. The steps of this removal process are schematized in Fig.~\ref{fig_6}.
...
The force threshold for complete fibril rupture increases with $T_s$, consistent with enhanced connectivity in more compact fibrils. Despite different rupture thresholds, all curves exhibit a common functional form:
...
During relaxation sweeps at a constant force level $F$, molecules detach either individually or in spatially connected groups. We define an avalanche size $s$ locally as the number of molecules in a spatially contiguous cluster (nearest-neighbor contacts on the lattice) that detach during the iterative sweeps at a fixed force $F$. While axial stress balance is computed across cross-sections (Eq.~\ref{eq2}), spatial cluster identification captures the local propagation of micro-damage, since the detachment of a molecule directly reduces the local coordination number $K$ of its immediate spatial neighbors and elevates their removal probability $P_R$ in subsequent sweeps. A spatially contiguous cluster of such detachments corresponds to a local avalanche event, a hallmark of crackling dynamics in driven disordered systems \cite{Jaeger1992,Godano1993,Beggs2003,Baraba1996,Alencar2001,Suki1994}. To quantify the importance of collective events, we introduce $\Psi$, defined as the fraction of removed molecules that belong to clusters relative to the total number of removed molecules at a given force. To compare across different rupture forces, we express the force as a normalized variable $F_n = F/F_{\mathrm{rup}}$, where $F_{\mathrm{rup}}$ is the rupture force of each realization. As shown in Fig.~\ref{fig_8}, low-$T_s$ fibrils exhibit large $\Psi$ even at small $F_n$, indicating early collective failure, whereas high-$T_s$ fibrils primarily fail through isolated removals until close to rupture, when $\Psi$ rises sharply.
