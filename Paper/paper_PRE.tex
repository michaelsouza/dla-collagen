...
The formation of real fibrils is driven by electrostatic forces \cite{Parkinson1995} that favor lateral association and minimize exposed surface area \cite{Kadler1987,Parkinson1995}. To represent the finite opportunity for post-attachment relaxation during fibril growth, we introduce $T_s$ as a dimensionless kinetic control parameter. Conceptually, $T_s$ may be viewed as proportional to the ratio $\tau_{\mathrm{dep}}/\tau_{\mathrm{rel}}$, where $\tau_{\mathrm{dep}}$ is the characteristic interval between molecular incorporation events and $\tau_{\mathrm{rel}}$ is the timescale associated with a local surface rearrangement. This interpretation is motivated by experimental studies showing that temperature, collagen concentration, pH, ionic strength, and buffer composition affect nucleation, longitudinal and lateral growth, and the resulting fibril diameter \cite{Yang2009,Roeder2002,Gobeaux2008,Kalbitzer2018,Christiansen2000,Kadler1988}. For example, lowering the gelation temperature from $37\,^{\circ}\mathrm{C}$ to $32\,^{\circ}\mathrm{C}$ slows type I collagen assembly and produces fewer, longer, and thicker fibrils \cite{Yang2009}, while pH, phosphate concentration, and sodium chloride content regulate the lateral-growth phase that determines fibril diameter \cite{Kalbitzer2018}. Electrostatic regulation of fibril size and growth kinetics by pH and ionic strength has also been observed in type II collagen \cite{Morozova2018}. Thus, different physicochemical or biological conditions may shift the effective relaxation-to-growth balance represented by $T_s$, although the present coarse-grained model does not provide a one-to-one quantitative mapping between $T_s$ and any single experimental variable.

Operationally, this relaxation is implemented as a lateral surface-diffusion step after attachment: a newly incorporated molecule performs a random walk restricted to the $x$-$z$ plane (no movement along the $y$-axis), using the same rules described above. The molecule is allowed up to $T_s$ diffusion attempts to search the local surface and is placed at the position that minimizes its exposed area; if multiple positions are equivalent, the first one encountered is retained \cite{Garci1991}. Using this algorithm, we generated ensembles of fibrils for different values of $T_s$ to examine how post-attachment mobility controls fibril compactness, structure, and subsequent mechanical response.
...
From a physical standpoint, the model developed here provides a simple mechanism by which physicochemical assembly conditions control structural compactness and the resulting mechanical resilience of collagen fibrils. Future work should incorporate more realistic mechanics and time dependence---including elastic or viscoelastic deformation and evolving stiffness (e.g., due to cross-link dynamics or degradation)---to predict stress--strain behavior and track how the fiber modulus changes over time under load.
