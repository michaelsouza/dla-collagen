...
Since our rupture model is probabilistic, for each value of $T_s$ we analyzed an ensemble of $10$ distinct fibrils, each containing $3\times 10^4$ molecules, and performed $10^3$ independent rupture simulations per fibril. During each simulation, the force was increased gradually, and at each level we recorded the molecules removed from the backbone.

To quantify the mechanical resistance of the fibrils, we analyzed the evolution of the average fraction of removed molecules, denoted by $\varphi$, as a function of the applied force $F$ \cite{Parkinson1997}. As shown in Fig.~\ref{fig_7}(a), $\varphi$ increases monotonically with $F$, reflecting progressive intermolecular bond ruptures and subsequent loss of load-bearing connectivity.
...
The main panel in Fig.~\ref{fig_9}(a) shows the intermediate regime used for the straight-line fits. This descriptive procedure yields $\gamma=2.31\pm 0.05$ for $T_s = 2$ and $\gamma=2.80\pm0.04$ for $T_s=8192$. Between these limits, the fitted slope varies systematically with $T_s$ (Fig.~\ref{fig_9}b): it increases approximately linearly with $\log(T_s)$ for $T_s < 512$ and reaches a plateau for $T_s \geq 512$. However, visual linearity of binned data over approximately two orders of magnitude does not by itself establish a pure power law or the absence of a characteristic scale. We therefore do not interpret $\gamma$ as a validated critical exponent.
