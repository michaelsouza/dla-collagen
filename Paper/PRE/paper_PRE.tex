...
\begin{abstract}
In this work, we investigate the relationship between assembly kinetics, structural organization, and mechanical failure in collagen fibrils. We employ a three-dimensional diffusion-limited aggregation (DLA) model with a surface diffusion parameter, $T_s$, to generate fibrils with varying morphologies. We show that $T_s$ controls the compactness of the structure: increasing molecular mobility leads to a transition from open, fractal aggregates to dense fibrils with higher fractal dimension ($D_f$). By coupling this formation model with a probabilistic fracture simulation, we demonstrate that $D_f$ is a strong predictor of mechanical resilience. Fibrils with higher $D_f$ possess a more connected load-bearing backbone, resulting in greater tensile strength. Furthermore, we find that the rupture process proceeds via avalanche-like events, whose sizes follow a power-law distribution. The scaling exponents of these avalanches depend systematically on $T_s$, establishing a quantitative link between the initial assembly conditions and the statistical dynamics of failure.
\end{abstract}
...
\begin{figure}[h!]
	...
	\caption{Average fractal dimension $D_f$ as a function of the diffusion parameter $T_s$ on a linear--log plot. Error bars are shown in red. The fractal dimension increases from $D_f = 1.708 \pm 0.005$ at $T_s = 2$ to $D_f = 1.963 \pm 0.001$ for $T_s = 8192$.}
	\label{fig_3}
\end{figure}		
...
For a given force level $F$, we evaluate $P_{R}$ for every molecule in the backbone and compare it with an independent random number $u\in[0,1]$. If $u < P_{R}$, the molecule is removed. After each sweep, stresses and probabilities are recalculated for the remaining backbone, and the sweep is repeated until no additional removals occur. The applied force is then increased by $\Delta F = 0.5$, and the procedure is repeated. Rupture is defined as the appearance of a completely void cross-section, signaling failure of the load-bearing backbone. The steps of this removal process are schematized in Fig.~\ref{fig_6}.
...
This plateau behavior of the $\gamma$ exponent correlates directly with the fractal dimension $D_f$ observed in Fig.~\ref{fig_3}. For $T_s \ge 512$, we find $D_f \ge 1.90$, suggesting that the fibril cross-section approaches its maximum packing density. This behavior, and the stabilization of the fit exponents $\beta$ and $\alpha$ in Figs.~\ref{fig_7}B and C for the same $T_s$ region, indicate that in this limit, the influence of $T_s$ on the failure properties becomes irrelevant.
