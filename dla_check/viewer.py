...
print('vpython.__version__ = ', vpython.__version__)
    ...
    with open(fdat, 'r') as fid:
        for row in fid.readlines():
            ...
            if row[0] == 'add':
                ...
                if len(df) == 0 or uid not in df:
                    uid = int(row[1])
                    x = int(row[2])
                    y = int(row[3])
                    z = int(row[4])
                    df[uid] = df[uid] = {'p':[(x,y,z)], 'bind':True}
                else:
                    df[uid]['bind'] = True
            elif row[0] == 'move':
                uid = int(row[1])
...
for uid in df:
    x, y, z = df[uid]['p'][0]
    #p = vector(x, y, z)    
    b = box(pos=vector(x, y, z), length=1, width=1, height=18, color=color.green, visible=False)
    B[uid] = (b, p)
